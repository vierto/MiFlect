...
struct HomeReflection: View {
    ...
    @State private var titleReflection = ""
            ...
            ScrollView {
                if vm.savedRefEntities.isEmpty {
                    ...
                    ForEach(vm.savedRefEntities) { entity in
                        ...
                        NavigationLink(destination: EditReflection( currReflection: entity.reflectionTitle ?? "", isEditReflectionAction: $goToEditReflection )) {

                            VStack(alignment: .leading, spacing: 20) {
                                
                                VStack(alignment: .leading, spacing: 20) {
                                    HStack(spacing: 24) {
                                        Text(entity.reflectionType ?? "")
                                        .fontWeight(.semibold)
                                        .font(.caption)
                                        .tracking(0.36)
...
//                                        Text(String(entity.reflectionDate))
                                        Text("July 29, 2022")
                                        .font(.caption)
                                    ...
                                    VStack(alignment: .leading, spacing: 12) {
                                        ...
                                        Text(entity.reflectionTitle ?? "")
                                                .fontWeight(.semibold)
                                                .font(.title3)
                                                .frame(maxWidth: .infinity, alignment: .leading)

                                        Text(entity.reflectionDesc ?? "")
                                                .font(.subheadline)
                                                .frame(maxWidth: .infinity, alignment: .leading)
                                                .lineSpacing(5)
                ...
                NavigationLink(destination: EditReflection(vm: vm, currReflection: "", isEditReflectionAction: $goToEditReflection), isActive: $goToEditReflection) {
                    EmptyView()
                }
                .toolbar {
